...
for j = 1:3
    ...
    kbNh(j) = knh(j)*If/A*ri/r*w/H*10*4/3;
end
...
% F = 8/3/pi * (r^3 - ri^3)/(r^4 - ri^4)
    ...
    kbt(j) = kt(j)*If/A*ri/r*w/H*10*4/3;
end
...
for j = 1:length(wve)
    ...
    kbD(j) = kD(j)*If/A*ri/r*w/H*10*4/3;
end
...
for j = 1:length(wve)
    ...
    kbH(j) = kH(j)*If/A*ri/r*w/H*10*4/3;
end
...
plot(Dv,kD)
hold on
scatter([40,60,80],fkD)

figure
plot(Hv,kH)
hold on
scatter([0.12,0.145,0.17],fkH)
...
plot(Dv,kbD)
hold on
scatter([40,60,80],fkbD)

figure
plot(Hv,kbH)
hold on
scatter([0.12,0.145,0.17],fkbH)